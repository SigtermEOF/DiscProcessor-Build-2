...
namespace APS_DiscProcessor_Build_2
{
    class RCDProcessing
    {
        ...
        DBConnections dbConns10 = new DBConnections();
        DataGatheringAndProcessing dataGatheringAndProcessing10 = new DataGatheringAndProcessing();
        string sBreakPoint = string.Empty;
        Main_Form mainForm = null;
        TaskMethods taskMethods10 = new TaskMethods();
        InsertsOrUpdates insertsOrUpdates10 = new InsertsOrUpdates();

        public void PushRCDToRender(string sProdNum, DataSet dataSetMain, Main_Form mForm)
        {
            try
            {
                mainForm = mForm;
                string sDiscType = "RCD";
                string sSitting = "";
                ...
                dbConns10.SQLQuery(sDiscProcessorConnString, sCommText, dTblDiscOrders);
                ...
                if (dTblDiscOrders.Rows.Count > 0)
                {
                    ...
                    string sSearchPattern01 = "Label = 'RCDTemplateFile'";
                    ...
                    if (dRowGatheredPattern01.Length > 0)
                    {
                        ...
                        string sSearchPattern02 = "Label = 'RCDExportFileDropDir'";
                        DataRow[] dRowGatheredPattern02 = dataSetMain.Tables["dTblVariables"].Select(sSearchPattern02);

                        if (dRowGatheredPattern02.Length > 0)
                        {
                            string sRCDExportFileDropDir = Convert.ToString(dRowGatheredPattern02[0]["Value"]).Trim();
                            string sPathToSave = sRCDExportFileDropDir + sRefNum + @"_" + sProdNum + ".txt";

                            File.WriteAllText(sPathToSave, sExportDefFileText);

                            bool bDiscOrderUpdateSuccess = false;
                            sCommText = "UPDATE [DiscOrders] SET [Status] = '35' WHERE [RefNum] = '" + sRefNum + "' AND [FrameNum] = '" + sFrameNum + "' AND [DiscType] = 'RCD'";

                            dbConns10.SQLNonQuery(sDiscProcessorConnString, sCommText, ref bDiscOrderUpdateSuccess);
                            ...
                            else if (bDiscOrderUpdateSuccess != true)
                            {
                                ...
                                insertsOrUpdates10.UpdateDiscOrdersForErrors(sErrorDescription, sRefNum, sFrameNum, sDiscType, sSitting, bSittingBased);

                                sStatus = "90";

                                insertsOrUpdates10.UpdateDiscOrdersTableStatusFrameBased(sRefNum, sFrameNum, sStatus, sDiscType, sProdNum);
                            }
                        }
                        else if (dRowGatheredPattern02.Length == 0)
                        {
                            string sErrorDescription = "[" + DateTime.Now.ToString() + "][Failed to gather RCDExportFileDropDir from the Variables table.]";
                    ...
                    {
                        string sErrorDescription = "[" + DateTime.Now.ToString() + "][Failed to gather RCDTemplateFile data from the Variables table.]";
            ...
            {
                dbConns10.SaveExceptionToDB(ex);
            }
        }
        ...
        public void CheckForRenderedOrderItems(DataSet dataSetMain, Main_Form mForm)
        {
            try
            {
                ...
                DataTable dTblDiscOrders = new DataTable("dTblDiscOrders");
                string sCommText = "SELECT * FROM [DiscOrders] WHERE [Status] = '35' AND [DiscType] = 'RCD'";
                ...
                if (dTblDiscOrders.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dRowDiscOrders in dTblDiscOrders.Rows)
                    {
                        ...
                        dbConns10.SQLQuery(sDP2ConnString, sCommText, dTblOrderItems);

                        if (dTblOrderItems.Rows.Count > 0)
                        {
                            iOrderItemCount = dTblOrderItems.Rows.Count;

                            string sTempRenderedPath = @"\\hp2\vol1\jobs\cdsburn\RoesCD\" + sRefNum + @"\"; //Note: set this as a variable
                            ...
                            {
                                string sDPRenderedPath = @"\\hp2\vol1\jobs\cdsburn\DiscProcessor Renders\RCD\" + sRefNum + @"\";
                                ...
                                if (iMovedFileCount == iOrderItemCount)
                                {
                                    ...
                                    this.GatherAndSaveRCDMergeData(sRefNum, sCustNum, sProdNum, ref bUpdateMergeDataSuccess, sFrameNum);

                                    if (bUpdateMergeDataSuccess == true)
                                    {
                                        //gather nwp data
                                        bool bUpdateNWPDataSuccess = false;
                                        this.GatherAndSaveRCDNWPData(sDPRenderedPath, iQuantity, sUniqueID, sRefNum, sFrameNum, ref bUpdateNWPDataSuccess);
                                        ...
                                        if (bUpdateNWPDataSuccess == true)
                                        {
                                            ...
                                            dbConns10.SQLQuery(sDiscProcessorConnString, sCommText, dTblGatherMergeAndNWPFileDataFromDiscOrders);
                                            ...
                                            if (dTblGatherMergeAndNWPFileDataFromDiscOrders.Rows.Count > 0)
                                            {
                                                ...
                                                string sNWPFilePath = sDPRenderedPath + "RCD" + "_" + sRefNum + "_" + sFrameNum + ".NWP";

                                                File.WriteAllText(sMergeFilePath, sMergeFileData);

                                                string sSearchPattern01 = "GatherDiscType = 'RCD'";
                                                ...
                                                if (dRowGatheredPattern01.Length > 0)
                                                {
                                                    ...
                                                    if (bGenerateNWPFiles == true)
                                                    {
                                                        ...
                                                        dataGatheringAndProcessing10.GatherNWPFileVariables(ref sReadyDir1, ref sReadyDir2, ref sReadyDir3, ref sReadyDir4, ref sSendHereFile, ref bGatheredNWPFileVariablesSuccess, dataSetMain);
                                                        ...
                                                        if (bGatheredNWPFileVariablesSuccess == true)
                                                        {
                                                            ...
                                                            if (File.Exists(sNWPFilePath))
                                                            {
                                                                ...
                                                                {
                                                                    string sSearchPattern02 = "DiscType = 'RCD'";
                                                                ...
                                                                dbConns10.SQLNonQuery(sDiscProcessorConnString, sCommText, ref bUpdateDiscOrdersStatus);
                                                    ...
                                                    else if (bGenerateNWPFiles != true)
                                                    {
                                                        ...
                                                        taskMethods10.LogText(sText, mForm);                                                        
                                                    }
                                                }
                                                else if (dRowGatheredPattern01.Length == 0)
                                                {
        ...
        private void GatherAndSaveRCDMergeData(string sRefNum, string sCustNum, string sProdNum, ref bool bUpdateMergeDataSuccess, string sFrameNum)
        {
            try
            {
                ...
                dbConns10.CDSQuery(sCDSConnString, sCommText, dTblCustomer);
                ...
                dbConns10.CDSQuery(sCDSConnString, sCommText, dTblItems);
                ...
                sCommText = "UPDATE [DiscOrders] Set [MergeFileData] = '" + sMergeText + "' WHERE [ProdNum] = '" + sProdNum + "' AND [FrameNum] = '" + sFrameNum + "' AND [DiscType] = 'RCD'";

                dbConns10.SQLNonQuery(sDiscProcessorConnString, sCommText, ref bUpdateMergeDataSuccess);
        ...
        private void GatherAndSaveRCDNWPData(string sDPRenderedPath, int iQuantity, string sUniqueID, string sRefNum, string sFrameNum, ref bool bUpdateNWPDataSuccess)
        {
            try
            {
                ...
                string sCommText = "SELECT [Value] FROM [Variables] WHERE [Label] = 'RCDLabelFile'";

                dbConns10.SQLQuery(sDiscProcessorConnString, sCommText, dTblVariables01);
                ...
                dbConns10.SQLNonQuery(sDiscProcessorConnString, sCommText, ref bUpdateNWPDataSuccess);
