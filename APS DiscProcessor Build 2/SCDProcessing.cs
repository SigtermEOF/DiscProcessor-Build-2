...
namespace APS_DiscProcessor_Build_2
{
    class SCDProcessing
    {
        ...
        DBConnections dbConns08 = new DBConnections();
        DataGatheringAndProcessing dataGatheringAndProcessing08 = new DataGatheringAndProcessing();
        string sBreakPoint = string.Empty;
        Main_Form mainForm = null;
        TaskMethods taskMethods08 = new TaskMethods();
        LockFile lockFile08 = new LockFile();
        InsertsOrUpdates insertsOrUpdates08 = new InsertsOrUpdates();

        public void SCDGatherRenderInfo(string sProdNum, ref bool bInitialPass01, ref bool bCreated, DataTable dTblOrder, string sSitting, DataSet dataSetMain, Main_Form mForm)
        {
            try
            {
                mainForm = mForm;
                string sDiscType = "SCD";
                bool bSittingBased = true;

                string sSearchPattern01 = "Label = 'SCDRenderedPath'";
                ...
                {
                    string sSCDRenderedPath = Convert.ToString(dRowGatheredPattern01[0]["Value"]).Trim();
                    ...
                    if (!Directory.Exists(sSCDRenderedPath))
                    {
                        Directory.CreateDirectory(sSCDRenderedPath);
                    }
                    ...
                    string sCommText = "SELECT * FROM [DiscOrders] WHERE [ProdNum] = '" + sProdNum + "' AND [DiscType] = 'SCD' AND [Sitting] = '" + sSitting + "'";

                    dbConns08.SQLQuery(sDiscProcessorConnString, sCommText, dTblDiscOrders);
                    ...
                    if (dTblDiscOrders.Rows.Count > 0)
                    {
                        try
                        {
                            ...
                            dbConns08.CDSQuery(sCDSConnString, sCommText, dTblFrames);

                            DataTable dTblDP2Image = new DataTable("dTblDP2Image");
                            sCommText = "SELECT Path, Frame FROM DP2Image WHERE Lookupnum = '" + sProdNum + "'";

                            dbConns08.CDSQuery(sCDSConnString, sCommText, dTblDP2Image);
                            ...
                            foreach (DataRow dRowDiscOrders in dTblDiscOrders.Rows)
                            {
                                ...
                                if (bInitialPass01 == true)
                                {
                                    ...
                                    taskMethods08.LogText(sText, mForm);
                                ...
                                if (dTblFrames.Rows.Count > 0)
                                {
                                    ...
                                    if (dRowGatheredPattern04.Length > 0)
                                    {
                                        ...
                                        {
                                            lockFile08.LockTheFile(ref iBatchID, ref iJobID, ref iJobIDsNeeded, ref bLockedFile, ref bGatherIDsSuccess, dataSetMain);
                                        }
                                            ...
                                            foreach (DataRow dRowFrames in dRowGatheredPattern04)
                                            {
                                                ...
                                                if (dTblDP2Image.Rows.Count > 0)
                                                {
                                                    ...
                                                    if (dRowGatheredPattern05.Length > 0)
                                                    {
                                                        ...
                                                        taskMethods08.ImageExists(sPath, ref bExists);
                                                        ...
                                                        if (bExists == true)
                                                        {
                                                            foreach (string s in lLayouts)
                                                            {
                                                                ...
                                                                if (s == "COLOR.txt")
                                                                {
                                                                    ...
                                                                    taskMethods08.CheckForExportFileExistence(ref sExportDef, ref bHaveFile, sExportDefPath);

                                                                    if (bHaveFile == true)
                                                                    {
                                                                        iRenderedCount += 1;

                                                                        sRenderedImageLocation = sSCDRenderedPath + sRefNum + @"\" + sProdNum + @"\" + sSitting.Trim() + @"\" + iRenderedCount + "-" + sImageName + ".JPG";
                                                                        ...
                                                                        taskMethods08.GetGSFromCodes(sProdNum, sFrameNum, ref sGSBG, sSitting);

                                                                        if (sGSBG.Length > 0)
                                                                        {
                                                                            bool bGSFound = false;
                                                                            taskMethods08.VerifyGS(sGSBG, ref bGSFound);
                                                        ...
                                                        else if (bExists != true)
                                                        {
                                                            ...
                                                            insertsOrUpdates08.UpdateDiscOrdersForErrors(sErrorDescription, sRefNum, sSequence, sDiscType, sExportDefSitting, bSittingBased);

                                                            string sStatus = "90";

                                                            insertsOrUpdates08.UpdateDiscOrdersTableStatusFrameBased(sRefNum, sSequence, sStatus, sDiscType, sProdNum);

                                                            taskMethods08.RemoveOrphanedExportDefFiles(dTblSitting);
                                                            taskMethods08.RemoveOrphanedExportDefFiles(dTblOrder);
                        ...
                        catch (ObjectDisposedException odex)
                        {
                            ...
                            dbConns08.SQLNonQuery(sDiscProcessorConnString, sCommText, ref bSuccess);
                    ...
                    {
                        dataGatheringAndProcessing08.ExportDefProcessing(sProdNum, bGoodResults, dTblOrder, sDiscType, bSittingBased, dataSetMain, mForm);
                    }
            ...
            {
                dbConns08.SaveExceptionToDB(ex);
            }
        }
    }
}
